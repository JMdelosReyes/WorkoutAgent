...
class ManageExerciseUseCaseImpl(private val repo: ExerciseRepository) : ManageExerciseUseCase {
    ...
    override suspend fun createExercise(exercise: Exercise): Resource<Boolean> = repo.createExercise(exercise)

    override suspend fun editExercise(exercise: Exercise): Resource<Boolean> = repo.editExercise(exercise)

...
class UserRepositoryImpl : UserRepository {
    ...
    override suspend fun createCustomer(customer: Customer): Resource<Boolean> {
        ...
        for (goal in customer.goals) {
            val hashGoal = hashMapOf<String, Any>(
                "aim" to goal.aim,
                "description" to goal.description,
                "isAchieved" to goal.isAchieved,
        ...
        for (weight in customer.weights) {
            val hashWeight =
                hashMapOf<String, Any>("date" to weight.date, "weight" to weight.weight)
            weightsArray.add(hashWeight)
        }
        val data: HashMap<String, Any?> = hashMapOf(
            "birthday" to customer.birthday,
            "dni" to customer.dni,
            "genre" to customer.genre,
            "email" to customer.email,
            "name" to customer.name,
            "surname" to customer.surname,
            "goals" to goalsArray,
            "photo" to customer.photo,
            "height" to customer.height,
            "phone" to customer.phone,
            "role" to "CUSTOMER",
            "weightPerWeek" to customer.weightPerWeek,
            "weights" to weightsArray
        )
        ...
        FirebaseFirestore.getInstance().collection("users").document(trainerLoggedId)
            .update("customers", FieldValue.arrayUnion(postResult!!)).await()
    ...
    override suspend fun updateCustomer(customer: Customer): Resource<Boolean> {
        ...
        val data: HashMap<String, Any?> = hashMapOf(
            ...
            "weightPerWeek" to customer.weightPerWeek,
            "weights" to weightsArray,
            "formula" to customer.formula,
            "formulaType" to customer.formulaType
        )
    ...
    override suspend fun deleteCustomer(id: String): Resource<Boolean> {
        ...
        val resultData = FirebaseFirestore.getInstance()
            ...
            .get().await()
        for (document in resultData) {
            FirebaseFirestore.getInstance().collection("routines").document(document.id).delete()
                .await()
        }
        ...
        if (!searchUser.documents.isEmpty()) {
            FirebaseFirestore.getInstance().collection("users").document(searchUser.documents[0].id)
                .update("customers", FieldValue.arrayRemove(customerToBeDeleted)).await()
        }
    ...
    override suspend fun deleteLoggedCustomer(): Resource<Boolean> {
        ...
        for (document in resultData) {
            FirebaseFirestore.getInstance().collection("routines").document(document.id)
    ...
    override suspend fun createTrainer(trainer: Trainer): Resource<Boolean> {
        ...
        val data: HashMap<String, Any> = hashMapOf(
            "academicTitle" to trainer.academicTitle,
            "birthday" to trainer.birthday,
            "customers" to customers,
            "dni" to trainer.dni,
            "email" to trainer.email,
            "name" to trainer.name,
            "surname" to trainer.surname,
            "phone" to trainer.phone,
            "photo" to trainer.photo,
            "role" to trainer.role
        )
        val postResult = FirebaseFirestore.getInstance().collection("users").add(data).await()
        return Resource.Success(true)
    }
    ...
    override suspend fun updateTrainer(trainer: Trainer): Resource<Boolean> {
        ...
        FirebaseFirestore.getInstance().collection("users").document(trainer.id).update(data)
            .await()
        return Resource.Success(true)
    }
            ...
            if (custos is List<*>) {
                for (ref in custos) {
                    if (ref is DocumentReference) {
                        ...
                        if (customerdoc.getString("photo") != null) customer.photo =
                            customerdoc.getString("photo")!! else customer.photo = ""
    ...
    override suspend fun updateProfileTrainer(trainerEdited: Trainer): Resource<Boolean> {
        ...
        val data: HashMap<String, Any?> = hashMapOf(
            "birthday" to trainerEdited.birthday,
            "dni" to trainerEdited.dni,
            "email" to trainerEdited.email,
            "name" to trainerEdited.name,
            "surname" to trainerEdited.surname,
            "photo" to trainer.photo,
            "phone" to trainerEdited.phone,
            "role" to trainer.role,
            "customers" to customers,
            "academicTitle" to trainer.academicTitle
        )
    ...
    override suspend fun updateProfileAdmin(admin: Administrator): Resource<Boolean> {
        ...
        val data: HashMap<String, Any?> = hashMapOf(
            ...
            "name" to admin.name,
            "surname" to admin.surname,
            "photo" to admin.photo,

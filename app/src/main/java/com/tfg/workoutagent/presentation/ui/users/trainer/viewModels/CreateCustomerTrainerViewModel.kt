...
class CreateCustomerTrainerViewModel(private val manageCustomerTrainerUseCase: ManageCustomerTrainerUseCase) :
    ViewModel() {
                    ...
                    when (val photoUri = upl.uploadPhotoUser(dataPhoto!!)) {
                        is Resource.Success -> {
                            ...
                            val customer = Customer(
                                birthday = parseStringToDateBar(birthday.value)!!,
                                dni = dni.toUpperCase(),
                                email = email,
                                genre = genre,
                                name = name,
    ...
    private fun checkData(): Boolean {
        ...
        checkHeight()
        checkInitialWeight()
        return _birthdayError.value == "" && _dniError.value == "" && _emailError.value == "" && _nameError.value == "" && _surnameError.value == "" &&
                _photoError.value == "" && _heightError.value == "" && _initialWeightError.value == "" && _phoneError.value == "" && _genreError.value == ""
    }
    ...
    private fun checkHeight() {
        height.let {
            if (it <= 0) _heightError.value =
                "The height must be greater than 0 centimeters" else _heightError.value = ""
        }
    }

    private fun checkInitialWeight() {
        initialWeight.let {
            if (it <= 0.0) _initialWeightError.value =
                "The weight must be greater than 0.0 kilograms" else _initialWeightError.value = ""
        }
    }
}
